...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <movies123privkey> [label] [rescan=true]\n"
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Movies123 address or script");
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <movies123address>\n"
            "Reveals the private key corresponding to <movies123address>.");
...
Value dumppem(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "dumppem <movies123address> <filename> <passphrase>\n"
            "Dump the key pair corresponding to <movies123address> and store it as encrypted PEM file."
            + HelpRequiringPassphrase());
