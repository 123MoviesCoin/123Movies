...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("movies123-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #movies123TEST2\r");
            Send(hSocket, "WHO #movies123TEST2\r");
        } else {
            // randomly join #movies12300-#movies12305
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #movies123%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #movies123%02d\r", channel_number).c_str());
        }
